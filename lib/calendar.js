...
async function createBookingEvent(slot, lead, rep, bookingType) {
  ...
  // Personal booking-type links (bookingType is set) send Google's own native
  // invite too, so the event auto-lands in the recipient's calendar with
  // Accept/Decline/Maybe — matching how Atrya's schedule links behave.
  // Inbound (bookingType undefined) keeps its original behavior: 'none' by
  // default, so the prospect gets only the single branded HansePay email
  // instead of Google's generic invite, unless BOOKING_SEND_GOOGLE_INVITE
  // is explicitly set.
  const sendGoogleInvite = !!bookingType || process.env.BOOKING_SEND_GOOGLE_INVITE === 'true';

  const result = await calendar.events.insert({
    calendarId:            owner,
    requestBody:           event,
    conferenceDataVersion: 1,
    sendUpdates:           sendGoogleInvite ? 'all' : 'none',
  });
