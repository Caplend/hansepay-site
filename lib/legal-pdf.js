...
//  Design: clean, minimal, typographic — Inter embedded throughout
// ─────────────────────────────────────────────────────────────────────────────
...
// Renders mixed bold/italic/link segments starting at explicit (x, y)
function rSegs(pdf, segs, x, y, width, color) {
  if (!segs || !segs.length) return;
  segs.forEach((s, i) => {
    const last = (i === segs.length - 1);
    if (s.bold)        pdf.font(FONT_NAMES.semibold);
    else if (s.italic) pdf.font(FONT_NAMES.regular);  // Inter-Italic not included; use regular
    else               pdf.font(FONT_NAMES.regular);
    pdf.fillColor(s.link ? BLUE : color);
    const opts = { continued: !last, width, lineGap: 4 };
    if (i === 0) pdf.text(s.text, x, y, opts);
    else         pdf.text(s.text, opts);
  });
  // Restore defaults after inline
...
function stampHeader(pdf, doc) {
  ...
  // Wordmark "HansePay" — Inter SemiBold, NAVY
  pdf.font(FONT_NAMES.semibold).fontSize(13).fillColor(NAVY)
     .text('HansePay', ML + 26, 15, { lineBreak: false });
...
function renderDocTitle(pdf, doc) {
  // Document type label in small-caps style (uppercase, letter-spaced)
  if (doc.badge) {
  ...
  // Large title
  pdf.font(FONT_NAMES.bold).fontSize(26).fillColor(NAVY)
  ...
  // Separator line — full width, light
  pdf.moveDown(1.0);
  const ly = pdf.y;
  pdf.save();
  ...
  for (const b of blocks) {
    switch (b.t) {
      ...
      case 'h2': {
        ensureSpace(pdf, 44);
        pdf.moveDown(0.9);
        pdf.font(FONT_NAMES.semibold).fontSize(10.5).fillColor(NAVY)
