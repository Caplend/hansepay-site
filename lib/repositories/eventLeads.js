...
async function updateLeadStatus(id, status, customerId) {
  await pool.query(
    'UPDATE event_leads SET status = :status, status_at = :now, created_customer_id = COALESCE(:customerId, created_customer_id) WHERE id = :id',
    { id, status, now: new Date(), customerId: customerId || null }
  );
...
module.exports = { listImports, findImportById, createImportWithLeads, listLeadsForImport, findLeadById, updateLeadStatus, deleteImport };
