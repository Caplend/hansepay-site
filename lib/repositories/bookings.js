...
async function create(b, conn) {
  await (conn || pool).query(
    `INSERT INTO bookings (id, customer_id, created_at, slot, \`lead\`, status, notes, meet_link,
       event_id, rebook_token, cancel_token, assigned_to, booking_type, rebooked, rebooked_at, updated_at,
     ...
     VALUES (:id, :customerId, :createdAt, CAST(:slot AS JSON), CAST(:lead AS JSON), :status, :notes,
       :meetLink, :eventId, :rebookToken, :cancelToken, CAST(:assignedTo AS JSON), CAST(:bookingType AS JSON), :rebooked, :rebookedAt,
       :updatedAt, :cancelledAt, :cancelledBy, :cancelledByName)`,
    {
      ...
      assignedTo: b.assignedTo ? JSON.stringify(b.assignedTo) : null,
      bookingType: b.bookingType ? JSON.stringify(b.bookingType) : null, rebooked: b.rebooked ? 1 : 0,
...
async function createWithCustomerUpsert({ booking, lead, opts }) {
  return withTransaction(async (conn) => {
    ...
    const callLabel = opts.callLabel || 'Discovery call';
    await activitiesRepo.create({
      customerId: cust.id, type: 'booking',
      title: isNew ? 'New lead from booking' : 'Repeat booking',
      body: opts.slot ? `${callLabel} booked for ${opts.slot.label || opts.slot.startISO}` : `${callLabel} booked`,
      by: 'system',
    }, conn);
...
async function update(id, patch, conn) {
  ...
  await (conn || pool).query(
    `UPDATE bookings SET customer_id=:customerId, slot=CAST(:slot AS JSON), \`lead\`=CAST(:lead AS JSON),
       ...
       cancel_token=:cancelToken, assigned_to=CAST(:assignedTo AS JSON), booking_type=CAST(:bookingType AS JSON), rebooked=:rebooked,
     ...
     WHERE id = :id`,
    {
      ...
      bookingType: merged.bookingType ? JSON.stringify(merged.bookingType) : null,
