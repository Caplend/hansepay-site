...
// ── Shared header rows (logo + badge + accent line) — ALWAYS blue, every email ──
// badge: short uppercase label string
// pad: optional horizontal padding (default 32px for 560px cards, 36px for 600px)
function hdrRow(badge, pad) {
  pad = pad || 32;
  const logo = pad >= 36 ? 36 : 34;
  const title = pad >= 36 ? 22 : 20;
  return `
  <tr><td bgcolor="#0B1929" class="hp-header" style="background:#0B1929;background:linear-gradient(135deg,#060D1A 0%,#0F2540 55%,#163659 100%);padding:24px ${pad}px;">
    <table role="presentation" width="100%"><tr>
      <td valign="middle"><table role="presentation"><tr>
        <td valign="middle" style="padding-right:10px;">${logoPill(logo)}</td>
        <td valign="middle" color="#ffffff" class="hp-hdr-txt" style="font-family:${SERIF};font-size:${title}px;font-weight:400;color:#ffffff;letter-spacing:.01em;">HansePay</td>
      </tr></table></td>
      <td align="right" valign="middle"><span style="display:inline-block;background:rgba(141,189,230,.18);color:${BRAND.n200};font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;padding:6px 13px;border-radius:100px;">${badge}</span></td>
    </tr></table>
  </td></tr>
...
// ── Shared footer row — same on every email ──
function ftrRow(tagline, pad) {
  pad = pad || 32;
  return `
  <tr><td bgcolor="#0B1929" class="hp-footer" style="background:${BRAND.navy};padding:20px ${pad}px;">
    <table role="presentation" width="100%"><tr>
      <td valign="middle"><table role="presentation"><tr>
      ...
      <td align="right" valign="middle" style="font-size:11px;color:rgba(255,255,255,.45);">${esc(tagline)}</td>
    </tr></table>
  ...
  const timelineRows = steps.map((step, i) => {
    ...
    <tr>
      <td valign="top" width="36" style="padding-right:4px;">
        <table role="presentation" cellpadding="0" cellspacing="0">
          <tr>
            <td width="28" height="28" bgcolor="${BRAND.blue}" style="background:${BRAND.blue};border-radius:14px;text-align:center;vertical-align:middle;font-size:12px;font-weight:700;color:#ffffff;line-height:28px;">${i + 1}</td>
          </tr>
          ${!isLast ? `<tr><td align="center" style="padding:2px 0;"><table role="presentation" cellpadding="0" cellspacing="0" style="width:2px;margin:0 auto;"><tr><td height="22" bgcolor="${BRAND.n200}" style="background:${BRAND.n200};font-size:0;line-height:0;">&nbsp;</td></tr></table></td></tr>` : ''}
        </table>
      </td>
      <td valign="top" style="padding:2px 0 ${isLast ? '0' : '16'}px 10px;">
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 28px rgba(11,25,41,.10);">
  ...
  <!-- Header — always blue -->
  ${hdrRow(esc(t.badge), 36)}

  <!-- Approval hero strip -->
  <tr><td bgcolor="#f0f7ff" style="background:#f0f7ff;padding:30px 36px 26px;text-align:center;border-bottom:1px solid #dbeafe;">
    <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto 14px;">
      <tr>
        <td width="56" height="56" bgcolor="${BRAND.blue}" style="background:${BRAND.blue};border-radius:28px;text-align:center;vertical-align:middle;font-size:26px;font-weight:700;color:#fff;line-height:56px;">&#10003;</td>
      </tr>
    </table>
    <div style="font-size:20px;font-weight:700;color:${BRAND.navy};letter-spacing:-.01em;margin-bottom:8px;">${isDE ? 'Konto genehmigt' : 'You\'re approved'}</div>
    <div style="font-size:14px;color:${BRAND.ink2};margin-bottom:12px;">${co}</div>
    <table role="presentation" cellpadding="0" cellspacing="0" align="center">
      <tr>
        <td bgcolor="#dbeafe" style="background:#dbeafe;border-radius:100px;padding:5px 15px;">
          <span style="font-size:13px;font-weight:700;color:${BRAND.blue};letter-spacing:.05em;">${ref}</span>
  ...
  <tr><td style="padding:32px 36px 8px;">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 32px;"><tr><td align="center">
      <a href="${esc(url)}" style="display:inline-block;background:${BRAND.navy};color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:15px 36px;border-radius:100px;">${esc(t.loginBtn)}</a>
    </td></tr></table>
...
/**
 * renderKycInviteEmail({ recipientName, recipientEmail, companyName, inviterName, kycUrl, lang, isIndividual })
 * isIndividual: true → omit company name, use simpler personal copy
 * Returns { to, subject, html, text }
 */
function renderKycInviteEmail({ recipientName, recipientEmail, companyName, inviterName, kycUrl, lang, isIndividual }) {
  ...
  const preheader = l === 'de'
    ? (ind ? 'Bitte schließen Sie Ihre Identitätsverifizierung ab, um Ihr Konto zu aktivieren.'
           : `${inviterName || 'HansePay'} hat Sie zur Identitätsverifizierung eingeladen.`)
    : (ind ? 'Please complete your identity verification to activate your account.'
  ...
  const badgeLabel = ind
    ? (l === 'de' ? 'ID-Verifizierung' : 'ID Verification')
    : (l === 'de' ? 'KYC-Verifizierung' : 'KYC Verification');
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 28px rgba(11,25,41,.10);">
  ...
  <tr><td bgcolor="#0B1929" class="hp-header" style="background:#0B1929;background:linear-gradient(135deg,#060D1A 0%,#0F2540 55%,#163659 100%);padding:24px 32px;">
    <table role="presentation" width="100%"><tr>
      ...
      <td align="right" valign="middle"><span style="display:inline-block;background:rgba(141,189,230,.18);color:${BRAND.n200};font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;padding:6px 13px;border-radius:100px;">${badgeLabel}</span></td>
    </tr></table>
  </td></tr>
  <tr><td style="height:3px;background:linear-gradient(90deg,${BRAND.blue2},${BRAND.n200});line-height:3px;font-size:0;">&nbsp;</td></tr>
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 28px rgba(11,25,41,.10);">
  ...
  <tr><td bgcolor="#0B1929" style="background:#0B1929;background:linear-gradient(135deg,#060D1A 0%,#0F2540 55%,#163659 100%);padding:24px 32px;">
    <table role="presentation" width="100%"><tr>
      ...
      <td align="right" valign="middle"><span style="display:inline-block;background:rgba(141,189,230,.18);color:${BRAND.n200};font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;padding:6px 13px;border-radius:100px;">Transfer confirmed</span></td>
    </tr></table>
  </td></tr>
  <tr><td style="height:3px;background:${BRAND.blue};background:linear-gradient(90deg,${BRAND.blue2},${BRAND.n200});line-height:3px;font-size:0;">&nbsp;</td></tr>
...
/**
 * renderPasswordResetEmail({ firstName, email, code })
 * Returns { to, subject, html, text }
 */
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 28px rgba(11,25,41,.10);">
  ...
  <tr><td bgcolor="#0B1929" class="hp-header" style="background:#0B1929;background:linear-gradient(135deg,#060D1A 0%,#0F2540 55%,#163659 100%);padding:24px 32px;">
    <table role="presentation" width="100%"><tr>
      ...
      <td align="right" valign="middle"><span style="display:inline-block;background:rgba(141,189,230,.18);color:${BRAND.n200};font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;padding:6px 13px;border-radius:100px;">Password reset</span></td>
    </tr></table>
  </td></tr>
  <tr><td style="height:3px;background:linear-gradient(90deg,${BRAND.blue2},${BRAND.n200});line-height:3px;font-size:0;">&nbsp;</td></tr>

  <!-- Hero strip -->
  <tr><td bgcolor="#f0f7ff" style="background:#f0f7ff;border-bottom:1px solid #dbeafe;padding:28px 32px;text-align:center;">
    <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto 14px;">
      <tr><td width="48" height="48" bgcolor="${BRAND.blue}" style="background:${BRAND.blue};border-radius:24px;text-align:center;vertical-align:middle;line-height:48px;font-size:22px;color:#fff;">&#128274;</td></tr>
    </table>
    <div style="font-size:20px;font-weight:700;color:${BRAND.navy};letter-spacing:-.01em;margin-bottom:6px;">Reset your password</div>
    <div style="font-size:13.5px;color:${BRAND.ink3};">Use the code below — it expires in 15 minutes</div>
  </td></tr>

  <!-- Body -->
  <tr><td style="padding:32px 32px 24px;">
    <p style="margin:0 0 16px;font-size:16px;color:${BRAND.ink};">Hi ${name},</p>
    <p style="margin:0 0 26px;font-size:15px;line-height:1.6;color:${BRAND.ink2};">
      We received a request to reset the password for your HansePay account. Enter the code below in the reset form to choose a new password.
    </p>
    ...
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" style="background:#f7fafd;border:1px solid ${BRAND.line};border-radius:14px;padding:24px 40px;text-align:center;">
        <tr><td style="font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:${BRAND.ink3};padding-bottom:10px;">Your reset code</td></tr>
        <tr><td><div style="font-size:42px;font-weight:800;letter-spacing:12px;color:${BRAND.navy};font-family:${SERIF};line-height:1;">${codeStr}</div></td></tr>
        <tr><td style="padding-top:10px;font-size:12px;color:${BRAND.ink3};">Valid for 15 minutes</td></tr>
      </table>
    </td></tr></table>

    <table role="presentation" width="100%" style="background:#fffbeb;border-left:3px solid #f59e0b;border-radius:6px;margin:0 0 24px;">
      <tr><td style="padding:14px 18px;font-size:13px;line-height:1.6;color:#92400e;">If you didn't request a password reset, you can safely ignore this email. Your password won't change.</td></tr>
    </table>

    <p style="margin:0 0 24px;font-size:13.5px;line-height:1.6;color:${BRAND.ink3};">Questions? Reach us at <a href="mailto:${esc(supportEmail)}" style="color:${BRAND.blue2};text-decoration:none;">${esc(supportEmail)}</a></p>
...
// ─── KYC identity verified email ─────────────────────────────────────────────

/**
 * renderKycVerifiedEmail({ firstName, email, lang })
 * Sent after an individual's identity documents are confirmed by compliance.
 * Returns { to, subject, html, text }
 */
function renderKycVerifiedEmail({ firstName, email, lang }) {
  ...
  const preheader = isDE ? 'Ihre Identitätsprüfung war erfolgreich. Ihr Antrag läuft weiter.' : 'Your identity check was successful. Your application continues.';
  const footer    = isDE ? 'EU-regulierte grenzüberschreitende Zahlungen · Hamburg' : 'EU-regulated cross-border payments · Hamburg';

  const steps = isDE ? [
    { label: 'E-Mail bestätigt',      done: true },
    { label: 'Identität verifiziert', done: true, current: true },
    { label: 'Compliance-Prüfung',    done: false },
    { label: 'Konto aktiviert',       done: false },
  ] : [
    { label: 'Email verified',        done: true },
    { label: 'Identity verified',     done: true, current: true },
  ...
  const stepHtml = steps.map((s, i) => {
    ...
    const bg   = s.done ? BRAND.blue : '#e2e8f0';
    const col  = s.done ? '#fff' : BRAND.ink3;
    const conn = s.done ? BRAND.blue2 : '#e2e8f0';
    return `
    <tr>
      <td valign="top" width="36" style="padding-right:4px;">
        <table role="presentation" cellpadding="0" cellspacing="0">
          <tr><td width="28" height="28" bgcolor="${bg}" style="background:${bg};border-radius:14px;text-align:center;vertical-align:middle;font-size:13px;font-weight:700;color:${col};line-height:28px;">${s.done ? '&#10003;' : (i + 1)}</td></tr>
          ${!last ? `<tr><td align="center" style="padding:2px 0;"><table role="presentation" cellpadding="0" cellspacing="0" style="width:2px;margin:0 auto;"><tr><td height="22" bgcolor="${conn}" style="background:${conn};font-size:0;line-height:0;">&nbsp;</td></tr></table></td></tr>` : ''}
        </table>
      </td>
      <td valign="top" style="padding:2px 0 ${last ? '0' : '16'}px 10px;">
        <div style="font-size:14px;font-weight:${s.current ? '700' : '500'};color:${s.current ? BRAND.navy : (s.done ? BRAND.ink2 : BRAND.ink3)};">${esc(s.label)}${s.current ? `&nbsp;<span style="display:inline-block;background:rgba(30,78,128,.12);color:${BRAND.blue};font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;padding:2px 7px;border-radius:100px;vertical-align:middle;">${isDE ? 'Jetzt' : 'Now'}</span>` : ''}</div>
      </td>
    </tr>`;
  }).join('');
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 28px rgba(11,25,41,.10);">
  ...
  ${hdrRow(isDE ? 'Identität verifiziert' : 'Identity verified')}

  <!-- Hero -->
  <tr><td bgcolor="#f0f7ff" style="background:#f0f7ff;padding:30px 32px 26px;text-align:center;border-bottom:1px solid #dbeafe;">
    <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto 14px;">
      <tr><td width="56" height="56" bgcolor="${BRAND.blue}" style="background:${BRAND.blue};border-radius:28px;text-align:center;vertical-align:middle;font-size:26px;font-weight:700;color:#fff;line-height:56px;">&#10003;</td></tr>
    </table>
    <div style="font-size:20px;font-weight:700;color:${BRAND.navy};letter-spacing:-.01em;margin-bottom:6px;">${isDE ? 'Identität verifiziert' : 'Identity verified'}</div>
    <div style="font-size:13.5px;color:${BRAND.ink3};">${isDE ? 'Ihre Identitätsprüfung war erfolgreich.' : 'Your identity check was successful.'}</div>
  </td></tr>

  <!-- Body -->
  <tr><td style="padding:32px 32px 28px;">
    <p style="margin:0 0 12px;font-size:16px;color:${BRAND.ink};">${isDE ? `Hallo ${name},` : `Hi ${name},`}</p>
    <p style="margin:0 0 28px;font-size:14.5px;line-height:1.65;color:${BRAND.ink2};">${isDE
      ...
      : 'Your identity documents have been reviewed and verified. Your application is now moving into the compliance review phase — we\'ll be in touch soon.'}</p>

    <!-- Progress -->
    <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:16px;">${isDE ? 'Ihr Fortschritt' : 'Your progress'}</div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
      ${stepHtml}
    </table>

    <p style="margin:0 0 24px;font-size:13.5px;line-height:1.6;color:${BRAND.ink3};">${isDE ? 'Fragen? Schreiben Sie uns an' : 'Questions? Reach us at'} <a href="mailto:${esc(supportEmail)}" style="color:${BRAND.blue2};text-decoration:none;">${esc(supportEmail)}</a></p>
    <p style="margin:0 0 2px;font-size:15px;color:${BRAND.ink2};">${isDE ? 'Mit freundlichen Grüßen,' : 'Best regards,'}</p>
    <p style="margin:0;font-size:15px;font-weight:700;color:${BRAND.navy};">${isDE ? 'Das HansePay-Team' : 'The HansePay Team'}</p>
  </td></tr>
...
// ─── All verifications complete email ─────────────────────────────────────────

/**
 * renderAllVerificationsEmail({ firstName, email, lang, accountType, company })
 * Sent when all KYC + document checks are complete, before final account activation.
 * Returns { to, subject, html, text }
 */
function renderAllVerificationsEmail({ firstName, email, lang, accountType, company }) {
  ...
  const preheader = isDE
    ? 'Ihre Identität und alle Dokumente wurden verifiziert. Die Kontoaktivierung läuft.'
    : 'Your identity and all documents have been verified. Account activation is underway.';
  const footer    = isDE ? 'EU-regulierte grenzüberschreitende Zahlungen · Hamburg' : 'EU-regulated cross-border payments · Hamburg';
  ...
  const checkHtml = checks.map(c => `
    <tr>
      <td valign="top" width="32" style="padding:5px 10px 5px 0;">
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 28px rgba(11,25,41,.10);">
  ...
  ${hdrRow(isDE ? 'Prüfungen abgeschlossen' : 'Checks complete')}

  <!-- Hero -->
  <tr><td bgcolor="#f0f7ff" style="background:#f0f7ff;padding:30px 32px 26px;text-align:center;border-bottom:1px solid #dbeafe;">
    <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto 14px;">
      <tr><td width="56" height="56" bgcolor="${BRAND.navy}" style="background:${BRAND.navy};border-radius:28px;text-align:center;vertical-align:middle;font-size:20px;color:#fff;line-height:56px;">&#10003;&#10003;</td></tr>
    </table>
    <div style="font-size:20px;font-weight:700;color:${BRAND.navy};letter-spacing:-.01em;margin-bottom:6px;">${isDE ? 'Alles bestätigt' : 'All checks complete'}</div>
    <div style="font-size:13.5px;color:${BRAND.ink3};">${isDE ? 'Ihr Konto wird in Kürze aktiviert.' : 'Your account will be activated shortly.'}</div>
  </td></tr>
...
module.exports = { sendMail, renderBookingEmail, renderRegistrationEmail, renderOtpEmail, renderApprovalEmail, renderKycInviteEmail, renderKycVerifiedEmail, renderAllVerificationsEmail, renderTxOtpEmail, renderTransactionEmail, renderPasswordResetEmail, gmailConfigured };
