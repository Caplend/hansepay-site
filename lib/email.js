...
// hansepay-mark-logo.png — 128x128, white mark, transparent background.
// Generated with macOS sips from the exact SVG paths used on hansepay.de.
const LOGO_URL = PUBLIC_BASE + '/assets/hansepay-mark-logo.png';
...
// Logo pill — white mark on transparent background.
// Wraps the img in a td with explicit navy bgcolor so email clients composite
// the transparent PNG against dark navy (not against the white email body).
// The wrapper td is exactly the same size as the image so no visible square.
function logoPill(size) {
  size = size || 36;
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="display:inline-table;"><tr><td bgcolor="#0B1929" style="background-color:#0B1929;line-height:0;font-size:0;"><img src="${LOGO_URL}" width="${size}" height="${size}" alt="HansePay" style="display:block;width:${size}px;height:${size}px;" border="0"></td></tr></table>`;
}
