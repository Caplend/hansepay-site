...
function emailFtr(tagline) {
  ...
  <tr><td style="padding:20px 40px 24px;text-align:center;border-top:1px solid ${BRAND.line};">
    ...
    <p style="margin:5px 0 0;font-size:11px;color:#B8C4CE;">© ${new Date().getFullYear()} HansePay &bull; Hamburg, Germany</p>
  </td></tr>`;
}
...
function signoff(sig, team) {
  return `<p style="margin:0 0 2px;font-size:15px;color:${BRAND.ink2};text-align:center;">${esc(sig)}</p>
<p style="margin:0 0 32px;font-size:15px;font-weight:700;color:${BRAND.navy};text-align:center;">${esc(team)}</p>`;
}
...
// refreshToken, when passed, sends as that specific account (a sales rep's
// own connected Gmail) instead of the shared GOOGLE_REFRESH_TOKEN mailbox.
async function sendViaGmail(msg, refreshToken) {
...
// refreshToken + from, when passed together, send this specific email as a
// sales rep's own connected Gmail account (see lib/calendar.js's per-rep
// OAuth) rather than the shared HansePay mailbox — same visual effect as
// Atrya's per-interviewer gmail.send confirmations.
async function sendMail({ to, subject, html, text, replyTo, refreshToken, from }) {
  const msg = {
    to,
    from: from || fromAddress(),
    replyTo: replyTo || process.env.EMAIL_REPLY_TO || process.env.CALENDAR_OWNER_EMAIL || null,
    bcc: refreshToken ? null : (process.env.EMAIL_BCC || null), // a rep's own sent mail doesn't need a company BCC
    subject,
    html,
    text: text || htmlToText(html),
  };

  if (refreshToken || gmailConfigured()) {
    try {
      const data = await sendViaGmail(msg, refreshToken);
...
const COPY = {
  en: {
    ...
    footerTagline: 'EU-regulated cross-border payments',
  ...
  de: {
    ...
    footerTagline: 'EU-regulierte grenzüberschreitende Zahlungen',
    detailsTitle: 'Ihre Angaben',
  },
};

function gcalLink(booking, title) {
  try {
    ...
    const label = title || 'HansePay Discovery Call';
    return 'https://calendar.google.com/calendar/render?action=TEMPLATE' +
      '&text=' + encodeURIComponent(label) +
      '&dates=' + start + '/' + end +
      '&details=' + encodeURIComponent(booking.meetLink ? 'Join Google Meet: ' + booking.meetLink : label) +
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${esc(t.hi(name))}</p>
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${esc(t.intro(isRebook))}</p>
    ...
    <h2 style="margin:0 0 12px;font-size:15px;font-weight:700;color:${BRAND.navy};text-align:center;">${esc(t.expectTitle)}</h2>
    ...
    <div style="font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:10px;text-align:center;">${esc(t.detailsTitle)}</div>
    ${infoCard(detailRows)}
...
// ─── Send booking link to a CRM customer (manual, from the admin) ─────────
const BOOKING_LINK_COPY = {
  en: {
    subject:   (repName) => `Book a time with ${repName} — HansePay`,
    preheader: 'Pick a time that works for you — it only takes a minute.',
    hdrSub:    'Let\'s find a time',
    hi:        (n) => `Hi ${n},`,
    intro:     (repName, label) => `${repName} would like to schedule a ${label ? label.toLowerCase() + ' ' : ''}call with you. Pick a time that works — it only takes a minute.`,
    btn:       'Choose a time →',
    signoff:   'Best,',
    footerTagline: 'EU-regulated cross-border payments',
  },
  de: {
    subject:   (repName) => `Termin mit ${repName} vereinbaren — HansePay`,
    preheader: 'Wählen Sie einen passenden Termin — dauert nur eine Minute.',
    hdrSub:    'Lassen Sie uns einen Termin finden',
    hi:        (n) => `Hallo ${n},`,
    intro:     (repName, label) => `${repName} möchte gerne ${label ? 'ein ' + label + ' ' : ''}mit Ihnen vereinbaren. Wählen Sie einen passenden Termin — dauert nur eine Minute.`,
    btn:       'Termin wählen →',
    signoff:   'Beste Grüße,',
    footerTagline: 'EU-regulierte grenzüberschreitende Zahlungen',
  },
...
function renderBookingLinkEmail({ toEmail, toName, repName, bookingUrl, bookingTypeLabel, lang, note }) {
  const l = (lang === 'de') ? 'de' : 'en';
  const t = BOOKING_LINK_COPY[l];
  const name = esc(toName || (l === 'de' ? 'dort' : 'there'));

  const html = `<!DOCTYPE html>
<html lang="${l}"><head>${emailHead(l, esc(t.subject(repName)))}</head>
<body style="margin:0;padding:0;background:${BRAND.off};font-family:${SANS};color:${BRAND.ink};-webkit-font-smoothing:antialiased;">
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ${heroIcon(ICO.calendar, '#EBF2FA')}
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${esc(t.hi(name))}</p>
    <p style="margin:0 0 10px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${esc(t.intro(repName, bookingTypeLabel))}</p>
    ${note ? `<p style="margin:0 0 22px;font-size:14px;line-height:1.6;color:${BRAND.ink2};text-align:center;font-style:italic;">${esc(note)}</p>` : ''}

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:22px 0 8px;"><tr><td align="center">
      <a href="${esc(bookingUrl)}" style="display:inline-block;background:${BRAND.navy};color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:13px 30px;border-radius:100px;">${esc(t.btn)}</a>
    </td></tr></table>

    ${signoff(t.signoff, repName)}
  </td></tr>
  ...
  return { to: toEmail, subject: t.subject(repName), html, text: htmlToText(html) };
}

// ─── Internal team notification: a new call was booked ────────────────────
function renderTeamBookingNotificationEmail({ teamEmail, lead, slot, repName, bookingTypeLabel, adminUrl }) {
  const when = formatWhen(slot.startISO, 'en');
  const name = esc(`${lead.firstName || ''} ${lead.lastName || ''}`.trim() || lead.email || 'Someone');
  const subject = `New booking: ${name} — ${when.datePart}`;
  ...
  const rows = [
    ...
    ['Booked with', repName || '—'],
    ['Call type', bookingTypeLabel || '—'],
    ['When', `${when.full} (Berlin time)`],
  ].filter(r => r[1] && r[1] !== '—' ? true : true);

  const html = `<!DOCTYPE html>
<html lang="en"><head>${emailHead('en', esc(subject))}</head>
<body style="margin:0;padding:0;background:${BRAND.off};font-family:${SANS};color:${BRAND.ink};-webkit-font-smoothing:antialiased;">
<div style="display:none;max-height:0;overflow:hidden;opacity:0;">A new call was just booked on the platform.</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.off};padding:32px 16px;">
<tr><td align="center">
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">

  ${emailHdr('New booking')}

  <tr><td style="padding:36px 40px 28px;">
    ${heroIcon(ICO.checkGn, '#EAF7F0')}
    <p style="margin:0 0 22px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">A new call was just booked on the platform.</p>

    ${infoCard(rows)}

    ${adminUrl ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:26px 0 0;"><tr><td align="center">
      <a href="${esc(adminUrl)}" style="display:inline-block;background:${BRAND.navy};color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:13px 28px;border-radius:100px;">View in CRM →</a>
    </td></tr></table>` : ''}
  </td></tr>

  ${emailFtr('EU-regulated cross-border payments')}

</table>
</td></tr></table>
</body></html>`;

  return { to: teamEmail, subject, html, text: htmlToText(html) };
}

// ─── CRM follow-up reminder (daily check, to the customer's owner) ────────
function renderFollowUpReminderEmail({ toEmail, ownerName, customer, crmUrl }) {
  const company = customer.company || 'this customer';
  const name = `${customer.firstName || ''} ${customer.lastName || ''}`.trim();
  const subject = customer.taskTitle ? `Task due today: ${customer.taskTitle}` : `Follow-up due today: ${company}`;

  const rows = [
    ['Task', customer.taskTitle || '—'],
    ['Company', customer.company || '—'],
    ['Contact', name || '—'],
    ['Email', customer.email || '—'],
    ['Stage', customer.stage || '—'],
  ].filter(r => r[1] !== '—' || r[0] === 'Task' || r[0] === 'Company');

  const html = `<!DOCTYPE html>
<html lang="en"><head>${emailHead('en', esc(subject))}</head>
<body style="margin:0;padding:0;background:${BRAND.off};font-family:${SANS};color:${BRAND.ink};-webkit-font-smoothing:antialiased;">
<div style="display:none;max-height:0;overflow:hidden;opacity:0;">A follow-up you scheduled in the CRM is due today.</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.off};padding:32px 16px;">
<tr><td align="center">
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">

  ${emailHdr('Follow-up reminder')}

  <tr><td style="padding:36px 40px 28px;">
    ${heroIcon(ICO.alert, '#FFF7E6')}
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">Hi ${esc(ownerName || 'there')},</p>
    <p style="margin:0 0 22px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${customer.taskTitle ? `A task you set in the CRM is due today: <strong>${esc(customer.taskTitle)}</strong> — for ${esc(company)}.` : `You set a follow-up date in the CRM for today — here's a reminder to reach out to ${esc(company)}.`}</p>

    ${infoCard(rows)}

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:26px 0 0;"><tr><td align="center">
      <a href="${esc(crmUrl)}" style="display:inline-block;background:${BRAND.navy};color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:13px 28px;border-radius:100px;">Open in CRM →</a>
    </td></tr></table>
  </td></tr>
  ...
  return { to: toEmail, subject, html, text: htmlToText(html) };
}

// ─── Free-text CRM email (sent from the customer drawer) ───────────────────
// Deliberately minimal — a branded wrapper around whatever the rep types,
// not a structured template. Paragraphs are split on blank lines.
function renderCrmComposeEmail({ toEmail, toName, fromName, subject, bodyText }) {
  const paragraphs = String(bodyText || '').split(/\n\s*\n/).map(p =>
    `<p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:${BRAND.ink};">${esc(p).replace(/\n/g, '<br>')}</p>`
  ).join('');
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ${paragraphs}
    ${signoff('Best,', fromName || 'The HansePay Team')}
  </td></tr>
...
// ─── Personal booking-link confirmation ───────────────────────────────────
// Deliberately separate from renderBookingEmail above — that template's copy
// ("30-minute discovery call", "your FX needs") is specific to the inbound
// flow, which must stay untouched. This one is generic: label/duration/
// description all come from the rep's own booking-type config.

const PERSONAL_COPY = {
  en: {
    subject:   (repName, d) => `Your call with ${repName} is confirmed — ${d}`,
    preheader: 'Your call is booked. Here are the details.',
    hdrSub:    'Call confirmed',
    hi:        (n) => `Hi ${n},`,
    intro:     (repName) => `Thanks for booking a call with ${repName}. Here are your details — a Google Meet link is attached.`,
    when: 'When', where: 'Where',
    meet:        'Google Meet — link below and in your calendar invite',
    joinBtn:     'Join the Google Meet',
    addCal:      'Add to calendar',
    calNote:     'Add the call to your calendar so you don\'t miss it.',
    signoff:     'See you soon,',
    footerTagline: 'EU-regulated cross-border payments',
    cancelLabel: 'Cancel this call',
  },
  de: {
    subject:   (repName, d) => `Ihr Termin mit ${repName} ist bestätigt — ${d}`,
    preheader: 'Ihr Termin ist gebucht. Hier die Details.',
    hdrSub:    'Termin bestätigt',
    hi:        (n) => `Hallo ${n},`,
    intro:     (repName) => `Vielen Dank für Ihre Buchung mit ${repName}. Hier sind Ihre Termindetails — ein Google-Meet-Link ist beigefügt.`,
    when: 'Wann', where: 'Wo',
    meet:        'Google Meet — Link unten und in Ihrer Kalendereinladung',
    joinBtn:     'Google Meet beitreten',
    addCal:      'Zum Kalender hinzufügen',
    calNote:     'Fügen Sie den Termin Ihrem Kalender hinzu.',
    signoff:     'Bis bald,',
    footerTagline: 'EU-regulierte grenzüberschreitende Zahlungen',
    cancelLabel: 'Termin stornieren',
  },
};

function renderPersonalBookingEmail(booking, bookingType, rep) {
  const lead      = booking.lead || {};
  const lang      = (lead.lang === 'de') ? 'de' : 'en';
  const t         = PERSONAL_COPY[lang];
  const when      = formatWhen(booking.slot.startISO, lang);
  const name      = esc(lead.firstName || (lang === 'de' ? 'dort' : 'there'));
  const repName   = (rep && rep.name) || 'HansePay';
  const meetLink  = booking.meetLink || booking.calendarUrl || null;
  const addCalUrl = gcalLink(booking, `${bookingType.label} · HansePay`);
  const tzLabel   = lang === 'de' ? '(Hamburger Zeit)' : '(Berlin time)';
  const durationLabel = lang === 'de' ? `${bookingType.duration} Minuten` : `${bookingType.duration} minutes`;
  const supportEmail = (rep && rep.calendarId) || process.env.EMAIL_REPLY_TO || process.env.CALENDAR_OWNER_EMAIL || 'hello@hansepay.de';
  const subject = t.subject(repName, when.datePart);
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ${heroIcon(ICO.calendar, '#EBF2FA')}
    ${heroH(esc(bookingType.label), lang === 'de' ? 'bestätigt' : 'confirmed')}
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${esc(t.hi(name))}</p>
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${esc(t.intro(repName))}</p>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F9FC;border:1px solid ${BRAND.line};border-radius:12px;margin:0 0 22px;">
      <tr><td style="padding:22px 24px;">
        <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:4px;">${esc(t.when)}</div>
        <div style="font-size:18px;font-weight:700;color:${BRAND.navy};margin-bottom:2px;">${esc(when.datePart)}</div>
        <div style="font-size:15px;color:${BRAND.blue2};margin-bottom:16px;">${esc(when.timePart)} ${esc(tzLabel)} · ${esc(durationLabel)}</div>
        <div style="border-top:1px solid ${BRAND.line};padding-top:14px;font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:4px;">${esc(t.where)}</div>
        <div style="font-size:14px;color:${BRAND.ink2};">${esc(t.meet)}</div>
      </td></tr>
    </table>
    ...
    ${bookingType.description ? `<p style="margin:0 0 26px;font-size:14px;line-height:1.6;color:${BRAND.ink2};">${esc(bookingType.description)}</p>` : ''}

    ${booking.cancelUrl ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px;"><tr><td align="center">
      <a href="${esc(booking.cancelUrl)}" style="font-size:12px;color:${BRAND.ink3};text-decoration:underline;">${esc(t.cancelLabel)}</a>
    </td></tr></table>` : ''}

    ${supportLine(lang === 'de' ? 'Hilfe benötigt? Kontaktieren Sie uns unter' : 'Need help? Contact us at', 'support@hansepay.de')}
    ${signoff(t.signoff, repName)}
  </td></tr>
  ...
  return { to: lead.email, subject, html, text: htmlToText(html) };
}
...
const REG_COPY = {
  en: {
    ...
    team:      'The HansePay Team',
    footer:    'EU-regulated cross-border payments',
  ...
  de: {
    ...
    questions: 'Hilfe benötigt? Kontaktieren Sie uns unter',
    signoff:   'Mit freundlichen Grüßen,',
    team:      'Ihr HansePay-Team',
    footer:    'EU-regulierte grenzüberschreitende Zahlungen',
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${esc(t.hi(name))}</p>
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${esc(t.intro)}</p>

    <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:14px;text-align:center;">${esc(t.nextTitle)}</div>
    ${stepList(t.steps, 0)}

    ${infoCard(detailRows)}

    ${supportLine(t.questions, 'support@hansepay.de')}
    ${signoff(t.signoff, t.team)}
  </td></tr>
...
function renderOtpEmail({ firstName, email, code, lang, verifyUrl }) {
  ...
  const footer    = isDE ? 'EU-regulierte grenzüberschreitende Zahlungen' : 'EU-regulated cross-border payments';
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${isDE ? `Hallo ${name},` : `Hi ${name},`}</p>
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${isDE
    ...
    ${supportLine(isDE ? 'Hilfe benötigt? Kontaktieren Sie uns unter' : 'Need help? Contact us at', 'support@hansepay.de')}
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:14px;text-align:center;">${isDE ? 'Nächste Schritte' : 'Next steps'}</div>
    ${stepList(steps)}
    ${supportLine(isDE ? 'Hilfe benötigt? Kontaktieren Sie uns unter' : 'Need help? Contact us at', 'support@hansepay.de')}
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${isDE ? `Hallo ${name},` : `Hi ${name},`}</p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${isDE
...
function renderKycInviteEmail({ recipientName, recipientEmail, firstName, lastName, companyName, inviterName, kycUrl, lang, isIndividual, role }) {
  ...
  const url  = kycUrl || '#';
  const footer = isDE
    ? 'EU-regulierte grenzüberschreitende Zahlungen'
    : 'EU-regulated cross-border payments';
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F9FC;border:1px solid ${BRAND.line};border-radius:12px;margin:0 0 24px;">
    <tr><td style="padding:16px 20px 6px;">
      <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:14px;text-align:center;">${isDE ? 'Ihre Angaben' : 'Your details'}</div>
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${isDE ? `Hallo ${fn},` : `Hi ${fn},`}</p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${isDE
...
// ─── KYC invite — individual account ─────────────────────────────────────

function renderKycInviteIndividualEmail({ recipientName, recipientEmail, firstName, kycUrl, lang }) {
  const l    = (lang === 'de') ? 'de' : 'en';
  const isDE = l === 'de';
  ...
  const subject   = isDE
    ? 'Schließen Sie Ihre Identitätsprüfung ab — HansePay'
    : 'Complete your identity verification — HansePay';
  const preheader = isDE
    ? 'Verifizieren Sie Ihre Identität, um Ihr HansePay-Konto zu aktivieren. Dieser Link läuft in 24 Stunden ab.'
    : 'Verify your identity to activate your HansePay account. This link expires in 24 hours.';

  const greeting = isDE
    ? (fn ? `Hallo ${esc(fn)},` : 'Hallo,')
    : (fn ? `Hi ${esc(fn)},` : 'Hi there,');

  const body = isDE
    ? `Um Ihr HansePay-Konto zu aktivieren, müssen Sie eine kurze Identitätsprüfung abschließen. Dies ist eine gesetzliche Anforderung und dauert nur wenige Minuten. Tippen Sie auf den Button unten, um Ihre Identität sicher über unseren vertrauenswürdigen Partner Signicat zu verifizieren.`
    : `To complete your HansePay account setup, you need to verify your identity. This is a regulatory requirement and takes just a few minutes.`;

  const whatToExpectSteps = isDE ? [
    { label: 'Klicken Sie auf den Button unten, um den sicheren Verifizierungsprozess zu starten.' },
    { label: 'Halten Sie Ihren Personalausweis oder Reisepass bereit.' },
    { label: 'Der Vorgang dauert in der Regel 3–5 Minuten.' },
  ] : [
    { label: 'Click the button below to start the secure verification process.' },
    { label: 'Have your government-issued ID or passport ready.' },
    { label: 'The process typically takes 3–5 minutes.' },
  ];
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${greeting}</p>
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${body}</p>

    <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:16px;text-align:center;">${isDE ? 'Was Sie erwartet' : 'What to expect'}</div>
    ${stepList(whatToExpectSteps)}

    ${ctaBtn(isDE ? 'Identität verifizieren →' : 'Verify my identity →', url)}
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:14px;text-align:center;">${isDE ? 'Ihr Fortschritt' : 'Your progress'}</div>
    ${stepList(steps, 2)}
...
// ─── KYC identity verification unsuccessful (individual) ──────────────────

function renderKycRejectedEmail({ firstName, email, lang, dashboardUrl }) {
  const l    = (lang === 'de') ? 'de' : 'en';
  const isDE = l === 'de';
  const name = esc(firstName || (isDE ? 'dort' : 'there'));
  ...
  const subject   = isDE
    ? 'Wir konnten Ihre Identität nicht verifizieren — HansePay'
    : 'We couldn\'t verify your identity — HansePay';
  const preheader = isDE
    ? 'Ihre Identitätsprüfung war leider nicht erfolgreich. Sie können es erneut über Ihr Dashboard versuchen.'
    : 'Your identity verification wasn\'t successful this time. You can request a new check from your dashboard.';
  const footer    = isDE
    ? 'EU-regulierte grenzüberschreitende Zahlungen'
    : 'EU-regulated cross-border payments';

  const reasons = isDE ? [
    'Das Foto Ihres Ausweisdokuments war unscharf oder unvollständig.',
    'Die angegebenen Daten stimmten nicht mit dem Dokument überein.',
    'Das hochgeladene Dokument ist abgelaufen.',
  ] : [
    'The photo of your ID document was unclear or incomplete.',
    'The details provided didn\'t match the document.',
    'The uploaded document has expired.',
  ];
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ${heroIcon(ICO.alert, '#FEF3E2')}
    ${heroH(isDE ? 'Verifizierung' : 'Verification', isDE ? 'nicht erfolgreich' : 'unsuccessful', '#D97706')}

    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${isDE ? `Hallo ${name},` : `Hi ${name},`}</p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${isDE
      ? 'Wir konnten Ihre Identität mit den eingereichten Angaben leider nicht bestätigen. Das passiert gelegentlich und ist meist schnell behoben — Sie können den Vorgang direkt über Ihr Dashboard erneut starten.'
      : 'We weren\'t able to confirm your identity with the details submitted. This happens sometimes and is usually quick to fix — you can start a new check right from your dashboard.'}</p>

    <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:14px;text-align:center;">${isDE ? 'Häufige Gründe' : 'Common reasons'}</div>
    <table role="presentation" width="100%" style="margin:0 0 26px;">
      ${reasons.map(r => `<tr>
        <td valign="top" style="width:20px;padding:3px 0;color:${BRAND.ink3};font-size:14px;">·</td>
        <td style="font-size:14px;line-height:1.55;color:${BRAND.ink2};padding:3px 0;">${esc(r)}</td>
      </tr>`).join('')}
    </table>

    ${ctaBtn(isDE ? 'Erneut versuchen →' : 'Request a new check →', url)}
...
// ─── KYC identity verification unsuccessful (company team member) ─────────
// Sent to a UBO/manager whose own verification failed as part of someone
// else's company application. Unlike renderKycRejectedEmail (individual
// accounts), this person usually has no HansePay login of their own — so
// there's no "go to your dashboard" CTA, just reassurance that the company's
// applicant has been notified and will send a fresh verification link.

function renderKycRejectedTeamMemberEmail({ recipientName, recipientEmail, companyName, adminName, lang }) {
  const l    = (lang === 'de') ? 'de' : 'en';
  const isDE = l === 'de';
  const name = esc(recipientName || (isDE ? 'dort' : 'there'));
  const co   = esc(companyName || (isDE ? 'das Unternehmen' : 'the company'));
  const admin = esc(adminName || (isDE ? 'der Antragsteller' : 'the applicant'));
  ...
  const subject   = isDE
    ? `Wir konnten Ihre Identität nicht verifizieren — ${companyName || 'HansePay'}`
    : `We couldn't verify your identity — ${companyName || 'HansePay'}`;
  const preheader = isDE
    ? 'Ihre Identitätsprüfung war leider nicht erfolgreich. Sie erhalten in Kürze einen neuen Link.'
    : 'Your identity verification wasn\'t successful this time. You\'ll receive a new link shortly.';
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${isDE ? `Hallo ${name},` : `Hi ${name},`}</p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${isDE
      ? `Wir konnten Ihre Identität im Rahmen des Antrags von <strong style="color:${BRAND.ink};font-weight:600;">${co}</strong> leider nicht bestätigen. Das passiert gelegentlich, zum Beispiel bei einem unscharfen Foto oder einem abgelaufenen Dokument.`
      : `We weren't able to confirm your identity as part of <strong style="color:${BRAND.ink};font-weight:600;">${co}</strong>'s application. This happens sometimes — for example, an unclear photo or an expired document.`}</p>

    <table role="presentation" width="100%" style="background:#F7F9FC;border-left:3px solid ${BRAND.blue2};border-radius:6px;margin:0 0 24px;">
      <tr><td style="padding:14px 18px;font-size:14px;line-height:1.6;color:${BRAND.ink2};">${isDE
        ? `Keine Aktion Ihrerseits erforderlich. <strong style="color:${BRAND.ink}">${admin}</strong> wurde informiert und wird Ihnen in Kürze einen neuen Verifizierungslink senden.`
        : `No action is needed from you right now. <strong style="color:${BRAND.ink}">${admin}</strong> has been notified and will send you a new verification link shortly.`}</td></tr>
    </table>
...
// ─── KYC identity verification unsuccessful (notify the onboarding admin) ──
// Sent to the person who submitted the company application (the "inviter"),
// when one of the UBOs/managers they added fails verification — tells them
// who failed and that they need to resend the invite.

function renderKycRejectedAdminEmail({ adminName, adminEmail, personName, personRole, companyName, lang, resendUrl }) {
  const l    = (lang === 'de') ? 'de' : 'en';
  const isDE = l === 'de';
  const admin = esc(adminName || (isDE ? 'dort' : 'there'));
  const person = esc(personName || (isDE ? 'Ein Teammitglied' : 'A team member'));
  const role   = esc(personRole || '');
  const co     = esc(companyName || (isDE ? 'Ihr Unternehmen' : 'your company'));
  const url    = resendUrl || '#';
  const supportEmail = process.env.EMAIL_REPLY_TO || process.env.CALENDAR_OWNER_EMAIL || 'support@hansepay.de';
  const footer = isDE
    ? 'EU-regulierte grenzüberschreitende Zahlungen'
  ...
  const subject   = isDE
    ? `Handlung erforderlich: Verifizierung von ${personName || 'einem Teammitglied'} fehlgeschlagen`
    : `Action needed: ${personName || 'A team member'}'s verification was unsuccessful`;
  const preheader = isDE
    ? `${person}s Identitätsprüfung für ${co} war nicht erfolgreich. Bitte senden Sie den Verifizierungslink erneut.`
    : `${person}'s identity verification for ${co} wasn't successful. Please resend the verification link.`;
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F9FC;border:1px solid ${BRAND.line};border-radius:12px;margin:0 0 24px;">
    <tr><td style="padding:16px 20px 6px;">
      <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:14px;text-align:center;">${isDE ? 'Betroffene Person' : 'Person affected'}</div>
      ${detailRow(isDE ? 'Name' : 'Name', person)}
      ${role ? detailRow(isDE ? 'Funktion im Unternehmen' : 'Role in company', role) : ''}
      ${detailRow(isDE ? 'Unternehmen' : 'Company', co)}
    </td></tr>
  </table>`;
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ${heroIcon(ICO.alert, '#FEF3E2')}
    ${heroH(isDE ? 'Verifizierung' : 'Verification', isDE ? 'fehlgeschlagen' : 'unsuccessful', '#D97706')}

    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${isDE ? `Hallo ${admin},` : `Hi ${admin},`}</p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${isDE
      ? `Die Identitätsprüfung von <strong style="color:${BRAND.ink};font-weight:600;">${person}</strong> im Rahmen Ihres Antrags für ${co} war leider nicht erfolgreich. Bitte senden Sie den Verifizierungslink erneut, damit ${person} es noch einmal versuchen kann.`
      : `<strong style="color:${BRAND.ink};font-weight:600;">${person}</strong>'s identity verification as part of your application for ${co} wasn't successful. Please resend the verification link so they can try again.`}</p>

    ${detailsCard}

    ${ctaBtn(isDE ? 'Verifizierung erneut senden →' : 'Resend verification →', url)}

    <p style="margin:0 0 24px;font-size:13px;line-height:1.6;color:${BRAND.ink3};">${isDE
      ? `${person} wurde ebenfalls per E-Mail informiert und muss selbst nichts weiter tun, bis der neue Link gesendet wurde.`
      : `${person} has also been notified by email and doesn't need to do anything further until the new link is sent.`}</p>
  ...
  return { to: adminEmail, subject, html, text: htmlToText(html) };
}
...
function renderAllVerificationsEmail({ firstName, lastName, email, lang, accountType, company, loginUrl }) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F9FC;border:1px solid ${BRAND.line};border-radius:12px;margin:0 0 24px;">
    <tr><td style="padding:14px 18px 12px;">
      <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};padding-bottom:12px;border-bottom:1px solid ${BRAND.line};margin-bottom:12px;text-align:center;">${isDE ? 'Verifizierte Personen' : 'Verified individuals'}</div>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        <tr>
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${isDE ? `Hallo ${name},` : `Hi ${name},`}</p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${intro}</p>
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">Hi ${name},</p>
    ...
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">We received a request to authorise this transfer. Enter the code below to confirm — it expires in 3 minutes.</p>
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">${hi}</p>
    <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">Your transfer has been authorised and is being processed. Funds typically arrive within 1–2 business days.</p>
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:0 0 8px;font-size:16px;color:${BRAND.ink};text-align:center;">Hi ${name},</p>
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">We received a request to reset the password for your HansePay account. Enter the code below in the reset form to choose a new password.</p>
    ...
    ${supportLine("Need help? Contact us at", 'support@hansepay.de')}
    ${signoff('Best,', 'The HansePay Team')}
  </td></tr>
...
function renderCustomTemplate(template, vars) {
  ...
  const footerTagline = template.footerTagline || 'EU-regulated cross-border payments';
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  ${emailFtr(isDE ? 'EU-regulierte grenzüberschreitende Zahlungen' : 'EU-regulated cross-border payments')}
</table>
</td></tr></table>
</body></html>`;
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 0;">
    ...
    <p style="margin:0 0 28px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${isDE ? `Hallo ${name},` : `Hi ${name},`} ${isDE
  ...
  <tr><td style="padding:0 40px 28px;">
    <div style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:${BRAND.ink3};margin-bottom:12px;text-align:center;">${isDE ? 'Kostenvergleich' : 'Cost breakdown'}</div>
...
// ─── Calculator: confirm your email to unlock the PDF ──────────────────────
function renderCalculatorConfirmEmail({ toEmail, result, confirmUrl, lang }) {
  const l = (lang === 'de') ? 'de' : 'en';
  const fmtEUR = (n) => new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(n || 0);
  const t = l === 'de' ? {
    subject: `Bitte bestätigen — Ihre Ersparnis-Analyse (${fmtEUR(result.saving)})`,
    preheader: 'Ein Klick, dann senden wir Ihnen die vollständige Aufschlüsselung als PDF.',
    hdrSub: 'E-Mail bestätigen',
    title: `Ihre geschätzte Ersparnis: ${fmtEUR(result.saving)} pro Jahr`,
    body: 'Bestätigen Sie Ihre E-Mail-Adresse, damit wir Ihnen die vollständige Aufschlüsselung als PDF zusenden können.',
    btn: 'E-Mail bestätigen & PDF erhalten',
    note: 'Falls Sie diese Berechnung nicht angefordert haben, können Sie diese E-Mail ignorieren.',
    footerTagline: 'EU-regulierte grenzüberschreitende Zahlungen',
  } : {
    subject: `Please confirm — your savings analysis (${fmtEUR(result.saving)})`,
    preheader: 'One click, then we\'ll send you the full breakdown as a PDF.',
    hdrSub: 'Confirm your email',
    title: `Your estimated saving: ${fmtEUR(result.saving)} per year`,
    body: 'Confirm your email address and we\'ll send you the full breakdown as a PDF.',
    btn: 'Confirm email & get PDF',
    note: 'If you didn\'t request this calculation, you can safely ignore this email.',
    footerTagline: 'EU-regulated cross-border payments',
  };

  const html = `<!DOCTYPE html>
<html lang="${l}"><head>${emailHead(l, esc(t.subject))}</head>
<body style="margin:0;padding:0;background:${BRAND.off};font-family:${SANS};color:${BRAND.ink};-webkit-font-smoothing:antialiased;">
<div style="display:none;max-height:0;overflow:hidden;opacity:0;">${esc(t.preheader)}</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.off};padding:32px 16px;">
<tr><td align="center">
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ${heroIcon(ICO.checkGn, '#E9F7EF')}
    <p style="margin:0 0 8px;font-size:18px;font-weight:600;color:${BRAND.ink};text-align:center;">${esc(t.title)}</p>
    <p style="margin:0 0 10px;font-size:15px;line-height:1.65;color:${BRAND.ink2};text-align:center;">${esc(t.body)}</p>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:22px 0 8px;"><tr><td align="center">
      <a href="${esc(confirmUrl)}" style="display:inline-block;background:${BRAND.navy};color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:13px 30px;border-radius:100px;">${esc(t.btn)}</a>
    </td></tr></table>

    <p style="margin:18px 0 0;font-size:12.5px;line-height:1.6;color:${BRAND.ink3};text-align:center;">${esc(t.note)}</p>
  </td></tr>
  ...
  return { to: toEmail, subject: t.subject, html, text: htmlToText(html) };
}

// ─── Calculator: confirmed — here's your PDF ────────────────────────────────
function renderCalculatorPdfReadyEmail({ toEmail, result, pdfUrl, lang }) {
  const l = (lang === 'de') ? 'de' : 'en';
  const fmtEUR = (n) => new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(n || 0);
  const t = l === 'de' ? {
    subject: `Ihre Ersparnis-Analyse ist da (${fmtEUR(result.saving)})`,
    preheader: 'Die vollständige Aufschlüsselung als PDF liegt bereit.',
    hdrSub: 'Bestätigt',
    title: 'Vielen Dank — hier ist Ihre Analyse.',
    body: 'Wie besprochen, hier die vollständige Aufschlüsselung Ihrer geschätzten Ersparnis als PDF.',
    btn: 'PDF herunterladen',
    ctaTitle: 'Fragen zu Ihren Zahlen?',
    ctaBody: 'Ein kurzes Gespräch reicht, um zu sehen, wie viel Ihr Unternehmen konkret sparen kann.',
    ctaBtn: 'Gespräch buchen',
    footerTagline: 'EU-regulierte grenzüberschreitende Zahlungen',
  } : {
    subject: `Your savings analysis is ready (${fmtEUR(result.saving)})`,
    preheader: 'The full breakdown as a PDF is ready.',
    hdrSub: 'Confirmed',
    title: 'Thanks — here\'s your analysis.',
    body: 'As promised, here\'s the full breakdown of your estimated saving as a PDF.',
    btn: 'Download PDF',
    ctaTitle: 'Questions about your numbers?',
    ctaBody: 'A short call is all it takes to see exactly how much your company could save.',
    ctaBtn: 'Book a call',
    footerTagline: 'EU-regulated cross-border payments',
  };
  const bookUrl = `https://www.hansepay.de/gespraech/?landing_page=calculator-pdf`;
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 20px;">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:22px 0 8px;"><tr><td align="center">
      <a href="${esc(pdfUrl)}" style="display:inline-block;background:${BRAND.navy};color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:13px 30px;border-radius:100px;">${esc(t.btn)}</a>
    </td></tr></table>
  </td></tr>

  <tr><td style="padding:0 40px 32px;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND.off};border-radius:12px;">
      <tr><td style="padding:22px 24px;text-align:center;">
        <p style="margin:0 0 4px;font-size:14px;font-weight:600;color:${BRAND.ink};">${esc(t.ctaTitle)}</p>
        <p style="margin:0 0 14px;font-size:13px;color:${BRAND.ink2};line-height:1.55;">${esc(t.ctaBody)}</p>
        <a href="${esc(bookUrl)}" style="display:inline-block;background:transparent;color:${BRAND.navy};font-size:13.5px;font-weight:600;text-decoration:none;padding:10px 22px;border-radius:100px;border:1.5px solid ${BRAND.navy};">${esc(t.ctaBtn)}</a>
      </td></tr>
    </table>
  </td></tr>
...
// ─── Waitlist: confirm your spot ─────────────────────────────────────────────
function renderWaitlistConfirmEmail({ toEmail, name, position, referralCode, confirmUrl, lang }) {
  const l = (lang === 'de') ? 'de' : 'en';
  const t = l === 'de' ? {
    subject: `Bestätigen Sie Ihren Platz #${position} auf der Warteliste`,
    preheader: 'Ein Klick, dann sichern Sie sich Ihren Platz.',
    hdrSub: 'Warteliste bestätigen',
    title: `Sie sind auf Platz #${position}.`,
    body: 'Bestätigen Sie Ihre E-Mail-Adresse, um Ihren Platz auf der Warteliste zu sichern.',
    btn: 'Platz bestätigen',
    referralNote: `Jede Empfehlung, die sich bestätigt, bringt Sie 25 Plätze nach vorn. Ihr Code: ${referralCode}`,
    note: 'Falls Sie sich nicht angemeldet haben, können Sie diese E-Mail ignorieren.',
    footerTagline: 'EU-regulierte grenzüberschreitende Zahlungen',
  } : {
    subject: `Confirm your spot #${position} on the waitlist`,
    preheader: 'One click to lock in your spot.',
    hdrSub: 'Confirm your spot',
    title: `You're #${position} on the list.`,
    body: 'Confirm your email address to lock in your spot on the waitlist.',
    btn: 'Confirm my spot',
    referralNote: `Every confirmed referral moves you 25 spots forward. Your code: ${referralCode}`,
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  <tr><td style="padding:36px 40px 28px;">
    ...
    <p style="margin:18px 0 0;font-size:12.5px;line-height:1.6;color:${BRAND.ink3};text-align:center;">${esc(t.referralNote)}</p>
    <p style="margin:8px 0 0;font-size:11.5px;line-height:1.6;color:${BRAND.ink3};text-align:center;">${esc(t.note)}</p>
  </td></tr>
...
module.exports = {
  sendMail,
  renderBookingEmail,
  renderPersonalBookingEmail,
  renderBookingLinkEmail,
  renderTeamBookingNotificationEmail,
  renderFollowUpReminderEmail,
  renderCrmComposeEmail,
  renderCalculatorConfirmEmail,
  renderCalculatorPdfReadyEmail,
  renderWaitlistConfirmEmail,
  renderRegistrationEmail,
  renderOtpEmail,
  renderApprovalEmail,
  renderKycInviteEmail,
  renderKycInviteIndividualEmail,
  renderKycVerifiedEmail,
  renderKycRejectedEmail,
  renderKycRejectedTeamMemberEmail,
  renderKycRejectedAdminEmail,
