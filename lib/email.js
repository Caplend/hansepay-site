...
// hansepay-mark-email-header.png = white mark baked onto #0B1929 navy (no transparency)
// Renders correctly in ALL email clients including Gmail (which converts transparent→white).
const LOGO_EMAIL_HEADER = PUBLIC_BASE + '/assets/hansepay-mark-email-header.png';
...
// Logo for email dark headers/footers — white mark on navy background, no transparency.
// Works in Gmail, Apple Mail, Outlook and all major clients.
function logoPill(size) {
  size = size || 36;
  return `<img src="${LOGO_EMAIL_HEADER}" width="${size}" height="${size}" alt="HansePay" style="display:block;width:${size}px;height:${size}px;max-width:${size}px;" border="0">`;
}

// Shared <head> block for all email templates.
// color-scheme:light prevents Apple Mail dark-mode auto-inversion (which turns
// white text black and inverts our carefully-composited logo PNG).
