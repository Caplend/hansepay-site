...
const LOGO_DARK  = PUBLIC_BASE + '/assets/hansepay-mark-uploaded.png';       // dark mark — use in email headers (white-bg pill)
const LOGO_WHITE = PUBLIC_BASE + '/assets/hansepay-mark-uploaded-white.png'; // white mark on transparent
...
// Logo pill helper — dark mark in a small white rounded container, works in all email clients
function logoPill(size) {
  size = size || 36;
  return `<div style="display:inline-block;width:${size}px;height:${size}px;border-radius:${Math.round(size*0.28)}px;background:#ffffff;overflow:hidden;vertical-align:middle;"><img src="${LOGO_DARK}" width="${size}" height="${size}" alt="HansePay" style="display:block;width:${size}px;height:${size}px;"></div>`;
}
