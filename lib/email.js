...
function renderKycInviteEmail({ recipientName, recipientEmail, companyName, inviterName, kycUrl, lang, isIndividual }) {
  ...
  const subject = isDE
    ? `Bitte bestätigen Sie Ihre Identität — ${ind ? 'HansePay' : (companyName || 'HansePay')}`
    : `One quick step before you're set — ${ind ? 'HansePay' : (companyName || 'HansePay')}`;
  const preheader = ind
    ? (isDE ? 'Nur noch ein kurzer Schritt — bestätigen Sie Ihre Identität und Sie sind startklar.'
            : 'Just one quick step — confirm your identity and you\'re good to go.')
    : (isDE ? `Ihr Konto für ${companyName || 'HansePay'} ist fast fertig — nur noch ein kurzer Schritt.`
            : `Your ${companyName || 'HansePay'} account is nearly ready — just one quick step left.`);
  const hdrSub = isDE ? 'Identität bestätigen' : 'Confirm your identity';
  const intro  = ind
    ? (isDE
        ? 'Wir müssen nur kurz bestätigen, dass Sie Sie sind — das dauert 3–5 Minuten und ist einmalig erforderlich, bevor Sie loslegen können.'
        : 'We just need to confirm who you are — it takes 3–5 minutes and only needs to happen once before you can get started.')
    : (isDE
        ? `<strong style="color:${BRAND.ink};font-weight:600;">${inv}</strong> hat Sie eingeladen, Ihre Identität für das <strong style="color:${BRAND.ink};font-weight:600;">${co}</strong>-Konto zu bestätigen. Das dauert nur 3–5 Minuten und muss einmalig gemacht werden.`
        : `<strong style="color:${BRAND.ink};font-weight:600;">${inv}</strong> has invited you to confirm your identity for the <strong style="color:${BRAND.ink};font-weight:600;">${co}</strong> account. It takes just 3–5 minutes and only needs to be done once.`);
  const steps = isDE
  ...
  const note = isDE
    ? 'Dieser Link ist sicher und persönlich für Sie — bitte nicht weitergeben. Er wird von unserem Verifizierungspartner Signicat bereitgestellt.'
    : 'This link is secure and personal to you — please don\'t share it. It\'s powered by our verification partner Signicat.';
  const footer = isDE ? 'EU-regulierte grenzüberschreitende Zahlungen · Hamburg' : 'EU-regulated cross-border payments · Hamburg';
...
<table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(11,25,41,.08);">
  ...
  ${emailHdr(isDE ? 'Identität bestätigen' : 'Confirm your identity')}
