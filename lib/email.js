...
const COPY = {
  en: {
    ...
    calNote: 'A calendar invitation with the Meet link is already in your inbox. Add it to your calendar so you don\'t miss it.',
  ...
  de: {
    ...
    calNote: 'Eine Kalendereinladung mit dem Meet-Link liegt bereits in Ihrem Postfach. Fügen Sie sie Ihrem Kalender hinzu, damit Sie den Termin nicht verpassen.',
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 28px rgba(11,25,41,.10);">
  ...
  <!-- Header -->
  <tr><td style="background:${BRAND.navy};padding:30px 36px;">
    <table role="presentation" width="100%"><tr>
      <td style="font-family:Georgia,'Times New Roman',serif;font-size:24px;font-weight:700;color:#ffffff;letter-spacing:-.01em;">HansePay</td>
      <td align="right"><span style="display:inline-block;background:rgba(201,169,97,.18);color:${BRAND.gold};font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;padding:6px 12px;border-radius:100px;">${esc(t.badge)}</span></td>
    </tr></table>
  </td></tr>
  ...
  <tr><td style="padding:36px 36px 8px;">
    ...
    ${meetLink ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 22px;"><tr><td align="center">
      <a href="${esc(meetLink)}" style="display:inline-block;background:${BRAND.blue};color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:14px 32px;border-radius:100px;">${esc(t.joinBtn)} →</a>
    </td></tr></table>` : ''}
    ...
    <table role="presentation" width="100%" style="margin:0 0 24px;">
      ${t.expect.map(item => `<tr>
        <td valign="top" style="width:22px;padding:3px 0;color:${BRAND.gold};font-size:15px;">●</td>
        <td style="font-size:14px;line-height:1.55;color:${BRAND.ink2};padding:3px 0;">${esc(item)}</td>
      </tr>`).join('')}
    </table>

    <!-- Prep -->
    <table role="presentation" width="100%" style="background:#fbf8f1;border-left:3px solid ${BRAND.gold};border-radius:6px;margin:0 0 24px;">
  ...
  <tr><td style="background:${BRAND.navy};padding:24px 36px;">
    <table role="presentation" width="100%"><tr>
      <td style="font-family:Georgia,serif;font-size:16px;font-weight:700;color:#ffffff;">HansePay</td>
      <td align="right" style="font-size:12px;color:rgba(255,255,255,.45);">${esc(t.footerTagline)}</td>
    </tr></table>
