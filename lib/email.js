...
// class="hp-logo" is required — the dark-mode @media block uses it to
// suppress Apple Mail's inversion filter (which would turn white → black).
function logoPill(size) {
  size = size || 36;
  return `<img src="${LOGO_WHITE_MARK}" width="${size}" height="${size}" alt="HansePay" class="hp-logo" style="display:block;width:${size}px;height:${size}px;max-width:${size}px;" border="0">`;
}

// Shared <head> block for all email templates.
// The @media (prefers-color-scheme: dark) block re-asserts our intended colors
// so Apple Mail dark mode cannot invert the white logo or header/footer text.
...
<style>
  @media (prefers-color-scheme: dark) {
    ...
    .hp-logo { filter: none !important; -webkit-filter: none !important; }
  }
</style>`;
}
