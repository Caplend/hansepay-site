...
function draftMessage(row, eventName) {
  const first = row.firstName || '';
  const event = eventName || 'der Veranstaltung';
  return `Hi ${first},\nich habe gesehen, dass ihr auch bei der ${event} dabei seid.\nIch bin seit Kurzem bei Atrya und würde die Gelegenheit gerne nutzen, spannende Unternehmen und ihre aktuellen Payment-Themen besser kennenzulernen.\nFalls du vor Ort bist, hast du Lust auf einen kurzen Kaffee und persönlichen Austausch?\nViele Grüße\nBesart`;
}
